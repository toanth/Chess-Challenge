...
namespace ChessChallenge.Example
{
    ...
    public class EvilBot : IChessBot
    {
        ...
        for (int depth = 0; depth++ < 50; )
            ...
            int square = bestRootMove.TargetSquare.Index;
            //passedPawnMask = (((0x7ul << (square % 8) >> 1) & 0xff) * 0x101010101010101) << (square / 8 * 8 + 8);
            //passedPawnMask = (((0x700ul << (square % 8) >> 1) & 0xff00) * 0x101010101010101) << (square / 8 * 8);
            //ulong passedPawnMask = (((0x700ul << square % 8 >> 1) & 0xff00) * 0x1010101010101) << (square & 0xf8);
            //ulong fileMask = (((0x700ul << square % 8 >> 1) & 0xff00) * 0x1010101010101);
            //bool isWhite = false;
            //ulong passedPawnMask = board.IsWhiteToMove ? fileMask << (square & 0xf8) : fileMask >> (8 + (square & 0xf8 ^ 56));
            ...
            foreach (bool stm in new[] { true, false })
            {
                for (var p = PieceType.None; ++p <= PieceType.King;)
                {
                    ...
                    while (mask != 0)
                    {
                        ...
                        square = BitboardHelper.ClearAndGetIndexOfLSB(ref mask) ^ (stm ? 56 : 0);
                        index = square + 64 * piece;
